...
function report() {
  ...
  for (let dieVal = 1; dieVal < 7; dieVal++) {  // we set up a nested loop, first we check for all the 1s, then 2s, etc
    ...
    if(howManyDice===3){
       document.querySelector("#report").innerHTML += `There are three of a kind of ${dieVal}s<br>`;
    }
    if(howManyDice===4){
       document.querySelector("#report").innerHTML += `There are four of a kind of ${dieVal}s<br>`;
    }
    if(howManyDice===5){
